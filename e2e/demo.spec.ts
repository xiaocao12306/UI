...
test("preempts dialog close button Enter via local key guard hook", async ({ page }) => {
  await page.goto("/");
...
test("preempts drawer close button Enter via local key guard hook", async ({ page }) => {
  await page.goto("/");
...
test("preempts command palette close button Enter via local key guard hook", async ({ page }) => {
  await page.goto("/");
...
test("preempts command palette close button key='Space' alias via local key guard hook", async ({
  page
}) => {
  await page.goto("/");
...
test("ignores repeated Escape keydown when palette clear-on-escape is disabled", async ({
  page
}) => {
  await page.goto("/");
...
test("preempts tabs key='Space' alias through local onTabKeyDown guard in lab panel", async ({
  page
}) => {
  await page.goto("/");
  ...
  await handoffTab.focus();
  await handoffTab.evaluate((element) => {
    element.dispatchEvent(
      new KeyboardEvent("keydown", { key: "Space", bubbles: true, cancelable: true })
    );
  });
...
test("preempts table sort key='Space' alias through local onSortKeyDown guard in lab panel", async ({
  page
}) => {
  await page.goto("/");
...
test("dismisses toast with Shift+Escape", async ({ page }) => {
  await page.goto("/");
