...
- Release evidence refresh: reran `demo:e2e` (`50` passed) and `release:dry-run` (`react` unpacked ~301.7 kB) with Storybook interaction baseline now `32/32` suites and `161/161` tests.
...
- Demo preemption parity: Playwright now validates global Escape preemption for Dialog + Drawer + Popover + CommandPalette + Dropdown, ensuring overlays stay open under app-level shortcut interception.
