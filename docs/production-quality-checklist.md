...
- Release evidence refresh: reran `demo:e2e` (`49` passed) and `release:dry-run` (`react` unpacked ~301.7 kB) with Storybook interaction baseline now `32/32` suites and `160/160` tests.
...
- Demo preemption parity: Playwright now validates global Escape preemption for Dialog + Drawer + CommandPalette + Dropdown, ensuring overlays stay open under app-level shortcut interception.
