...
- 已完成
  ...
  - Release Gate 证据刷新（2026-03-18：demo E2E `50` + Storybook `161/161` + dry-run `react` unpacked `~301.7 kB`）
  ...
  - Demo Escape 抢占验收扩展（Dialog/Drawer/Popover/CommandPalette/Dropdown 新增 Playwright 回归，锁定全局抢占后保持打开，解除抢占后恢复正常关闭）
