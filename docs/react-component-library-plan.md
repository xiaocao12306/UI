...
- 已完成
  ...
  - Pagination 键盘焦点迁移与测试时序稳定性收口（2026-03-22 latest+231：Pagination 的键盘翻页焦点迁移改为在目标页切换前同步记录目标 button 焦点态并在程序化 `focus()` 期间抑制冗余 blur/focus 状态抖动，避免连续键盘跳页出现可见状态闪烁；同时将边界键与焦点迁移相关用例补齐 `act` 包裹，消除测试中的非包裹状态更新噪音并保持交互断言稳定；同步 `Pagination.tsx`、`Pagination.test.tsx`，并复验 `pnpm --filter @aurora-ui/react exec vitest run src/Pagination.test.tsx`、`pnpm storybook:build`、`pnpm storybook:test:grep \"Pagination.stories.tsx\"` 与 docs parity 相关检查通过）
- 进行中
