import{j as o}from"./jsx-runtime-BjG_zV1W.js";import{B as oo}from"./Button-B21ouHNm.js";import{r as m}from"./index-BWu4c2F4.js";import{B as ro}from"./Badge-ZJmMstsz.js";import{within as w,expect as a,userEvent as y,fireEvent as g,waitFor as M}from"./index-DgAF9SIF.js";import{S as p,s as H,a as Ue}from"./storyShowcase-DlKirntE.js";const Pa="Enter Space",so=`${Pa} Home End PageDown PageUp ArrowLeft ArrowRight`,io="ArrowLeft ArrowRight Home End PageDown PageUp",co=48,lo=new Intl.Collator(void 0,{numeric:!0,sensitivity:"base"}),ht=400;function xt(e,t,n,r){if(!n)return null;const v=e.findIndex(L=>String(L.key)===n&&L.sortable);return v<0?null:{key:n,direction:r,renderKey:t[v]??`${n}__index-${v}`}}function ft(e,t,n){if(typeof e.sortLabel=="string"&&e.sortLabel.trim().length>0)return e.sortLabel.trim();const r=_e(at(e.header));return r.length>0?r:t}function St(e,t){const n=_e(e.trim().replace(/[_-]+/g," ").replace(/([a-z0-9])([A-Z])/g,"$1 $2"));return n.length>0&&/[0-9A-Za-z]/.test(n)?n:t>=0?`Column ${t+1}`:"Column"}function d({columns:e,data:t,rowKey:n,minTableWidth:r,caption:v,ariaLabel:L,ariaLabelledBy:R,loading:D=!1,loadingContent:F="Loading data...",emptyContent:E="No data available.",defaultSortKey:N,defaultSortDirection:Y="asc",getSortAriaLabel:Oa=Bt,getSortStatusText:ot=uo,onSortKeyDown:Ge,onSortChange:Ye}){const rt=Ve(v),$a=Ve(F),qa=Ve(E),za=$a?F:"Loading data...",Va=qa?E:"No data available.",Z=re(R),st=Z?void 0:re(L,rt?void 0:"Data table"),_a=po(r),O=m.useRef({}),Ze=m.useRef(null),Je=m.useRef(null),J=m.useRef(0),$=m.useRef(null),Q=m.useRef(null),q=m.useRef(!0);m.useRef(null),m.useRef(null),m.useRef(null);const[Ua,Qe]=m.useState(null),[Ga,W]=m.useState(null),[Ya,z]=m.useState(null),[Za,Xe]=m.useState(!1);m.useEffect(()=>{var c;const s=((c=Ze.current)==null?void 0:c.ownerDocument)??document,l=b=>{b.metaKey||b.altKey||b.ctrlKey||(q.current=!0)},u=b=>{"button"in b&&!nt(b.button)||"ctrlKey"in b&&b.ctrlKey||(q.current=!1)};return s.addEventListener("keydown",l,!0),s.addEventListener("pointerdown",u,!0),s.addEventListener("mousedown",u,!0),s.addEventListener("touchstart",u,!0),()=>{s.removeEventListener("keydown",l,!0),s.removeEventListener("pointerdown",u,!0),s.removeEventListener("mousedown",u,!0),s.removeEventListener("touchstart",u,!0)}},[]);const X=m.useCallback(()=>{Je.current=null,J.current=0,$.current!==null&&((Q.current??window).clearTimeout($.current),$.current=null),Q.current=null},[]);m.useEffect(()=>()=>{X()},[X]);const C=m.useMemo(()=>{const s=new Map;return e.map(l=>{const u=String(l.key),c=s.get(u)??0;return s.set(u,c+1),c===0?u:`${u}__duplicate-${c}`})},[e]),[h,ee]=m.useState(()=>xt(e,C,N,Y)),P=m.useMemo(()=>D||t.length<=1?[]:e.reduce((s,l,u)=>(l.sortable&&s.push(C[u]??`${String(l.key)}__index-${u}`),s),[]),[e,t.length,D,C]);m.useEffect(()=>{var c;if(!h)return;const s=C.findIndex(b=>b===h.renderKey);if(s>=0&&((c=e[s])!=null&&c.sortable))return;const l=e.findIndex(b=>b.sortable&&String(b.key)===h.key);if(l<0){ee(null);return}const u=C[l]??`${h.key}__index-${l}`;u!==h.renderKey&&ee(b=>b&&{...b,renderKey:u})},[e,C,h]),m.useEffect(()=>{if(h)return;const s=xt(e,C,N,Y);s&&ee(s)},[e,Y,N,C,h]);const et=m.useMemo(()=>t.map((s,l)=>String(n?n(s,l):l)),[t,n]),Ja=m.useMemo(()=>{const s=new Map;return et.map((l,u)=>{const c=s.get(l)??0;return s.set(l,c+1),c===0?l:`${l}__dup-${u}`})},[et]);m.useEffect(()=>{},[n,et]),m.useEffect(()=>{},[e]),m.useEffect(()=>{},[e]);const V=m.useMemo(()=>{const s=t.map((x,B)=>({row:x,sourceIndex:B}));if(!h)return s;const l=C.findIndex(x=>x===h.renderKey),u=l>=0?l:e.findIndex(x=>String(x.key)===h.key),c=u>=0?e[u]:void 0;if(!(c!=null&&c.sortable))return s;const b=c.sortAccessor??(x=>{const B=x[String(c.key)];return B instanceof Date||typeof B=="number"||typeof B=="string"?B:String(B??"")});return s.sort((x,B)=>{const A=b(x.row),T=b(B.row);if(typeof A=="string"&&typeof T=="string"){const U=lo.compare(A,T);return U!==0?h.direction==="asc"?U:-U:x.sourceIndex-B.sourceIndex}const K=A instanceof Date?A.getTime():A,te=T instanceof Date?T.getTime():T;return K<te?h.direction==="asc"?-1:1:K>te?h.direction==="asc"?1:-1:x.sourceIndex-B.sourceIndex})},[e,t,C,h]),Qa=typeof document>"u"?void 0:document,it=m.useMemo(()=>{if(D||V.length<=1||!h)return"";const s=C.findIndex(b=>b===h.renderKey),l=s>=0?s:e.findIndex(b=>String(b.key)===h.key),u=l>=0?e[l]:void 0;if(!(u!=null&&u.sortable))return"";const c=ft(u,St(h.key,l));return ot({columnKey:h.key,columnHeader:c,direction:h.direction})},[e,ot,D,Qa,C,h,V.length]),G=!D&&V.length>1&&e.some(s=>s.sortable),_=!G&&Za,ct=Math.max(e.length,1);return m.useEffect(()=>{G||(Qe(null),W(null),z(null))},[G]),m.useEffect(()=>{if(G){Xe(!1);return}const s=Ze.current;if(!s){Xe(!1);return}const l=s.ownerDocument.defaultView??(typeof window>"u"?void 0:window);if(!l)return;const u=()=>{const b=s.scrollWidth-s.clientWidth>0;Xe(x=>x===b?x:b)};u(),l.addEventListener("resize",u);let c=null;return typeof l.ResizeObserver=="function"&&(c=new l.ResizeObserver(u),c.observe(s)),()=>{l.removeEventListener("resize",u),c==null||c.disconnect()}},[e.length,t.length,G,D]),o.jsxs("div",{ref:Ze,"data-aurora-table-scroll-container":"",role:_?"region":void 0,tabIndex:_?0:void 0,"aria-label":_?Z?void 0:st??"Data table scroll container":void 0,"aria-labelledby":_?Z:void 0,"aria-keyshortcuts":_?io:void 0,onKeyDown:s=>{if(!_||s.defaultPrevented||s.altKey||s.ctrlKey||s.metaKey||s.target!==s.currentTarget)return;const l=s.currentTarget,u=Math.max(0,l.scrollWidth-l.clientWidth);if(u<=0)return;const c=l.clientWidth,b=Math.max(co,Math.floor(c*.3)),x=Rt(l),B=x==="rtl"?"ArrowLeft":"ArrowRight",A=K=>{K!==0&&(s.preventDefault(),wo(l,K))},T=K=>{s.preventDefault(),bo(l,K)};switch(s.key){case"ArrowLeft":case"ArrowRight":A(s.key===B?b:-b);return;case"PageDown":A(c);return;case"PageUp":A(-c);return;case"Home":T(x==="rtl"?u:0);return;case"End":T(x==="rtl"?0:u);return;default:return}},style:{border:"1px solid var(--aurora-border-default)",borderRadius:"var(--aurora-radius-md)",overflow:"auto"},children:[it?o.jsx("p",{role:"status","aria-live":"polite",style:{position:"absolute",width:1,height:1,padding:0,margin:-1,overflow:"hidden",clip:"rect(0, 0, 0, 0)",whiteSpace:"nowrap",border:0},children:it}):null,o.jsxs("table",{"aria-label":st,"aria-labelledby":Z,"aria-busy":D||void 0,style:{width:"100%",borderCollapse:"collapse",minWidth:_a},children:[rt?o.jsx("caption",{style:{textAlign:"left",padding:"10px 12px",color:"var(--aurora-text-secondary)",fontSize:"var(--aurora-font-size-sm)",borderBottom:"1px solid var(--aurora-border-default)"},children:v}):null,o.jsx("thead",{children:o.jsx("tr",{children:e.map((s,l)=>{const u=String(s.key),c=C[l]??`${u}__index-${l}`,b=!!s.sortable,x=V.length>1,B=b&&(h==null?void 0:h.key)===u?h.direction:void 0,A=B&&(h==null?void 0:h.renderKey)===c?B:void 0,T=!D&&x?A:void 0,K=T?T==="asc"?"ascending":"descending":void 0,te=s.align??"left",U=ft(s,St(u,l)),ne=B==="asc"?"desc":"asc",Xa=Oa({columnKey:u,columnHeader:U,nextDirection:ne}),lt=re(s.sortLabelledBy),eo=lt?void 0:re(Xa,Bt({columnHeader:U,nextDirection:ne})),j=D||!x,to=j?void 0:P.length>1?so:Pa,ut=!j&&Ua===c,dt=!j&&Ga===c,mt=!j&&Ya===c,no=ut||mt,yt=()=>{j||(ee({key:u,direction:ne,renderKey:c}),Ye==null||Ye(u,ne))};return o.jsx("th",{scope:"col","aria-sort":K,style:{textAlign:te,color:"var(--aurora-text-secondary)",background:"var(--aurora-surface-elevated)",borderBottom:"1px solid var(--aurora-border-default)",padding:"10px 12px",fontSize:"var(--aurora-font-size-sm)",fontWeight:"var(--aurora-font-weight-medium)",width:s.width},children:b?o.jsxs("button",{"data-aurora-reduced-motion":"transition",type:"button",ref:i=>{O.current[c]=i},"aria-labelledby":lt,"aria-label":eo,"aria-keyshortcuts":to,disabled:j,onClick:i=>{const k=Date.now()-J.current,ae=i.detail===0&&Je.current===c&&J.current>0&&k<=ht;X(),!ae&&yt()},onMouseEnter:()=>{j||Qe(c)},onMouseLeave:()=>{Qe(i=>i===c?null:i),W(i=>i===c?null:i)},onPointerDown:i=>{j||i.ctrlKey||nt(i.button)&&(q.current=!1,z(k=>k===c?null:k),W(c))},onPointerUp:i=>{nt(i.button)&&W(k=>k===c?null:k)},onMouseDown:i=>{j||i.button!==0||i.ctrlKey||(q.current=!1,z(k=>k===c?null:k),W(c))},onMouseUp:i=>{i.button===0&&W(k=>k===c?null:k)},onPointerCancel:()=>{W(i=>i===c?null:i)},onFocus:()=>{if(j){z(i=>i===c?null:i);return}z(i=>yo(O.current[c],q.current)?c:i===c?null:i)},onBlur:()=>{z(i=>i===c?null:i),W(i=>i===c?null:i)},onKeyDown:i=>{var ae,pt,wt,bt;if(q.current=!0,Ge==null||Ge(i),i.defaultPrevented||i.altKey||i.ctrlKey||i.metaKey)return;if(i.key==="Home"){const I=P[0];if(!I)return;i.preventDefault(),(ae=O.current[I])==null||ae.focus();return}if(i.key==="End"){const I=P[P.length-1];if(!I)return;i.preventDefault(),(pt=O.current[I])==null||pt.focus();return}if(i.key==="PageDown"||i.key==="PageUp"){const I=P.indexOf(c);if(I<0)return;const vt=i.key==="PageDown"?1:-1,oe=Math.max(0,Math.min(P.length-1,I+vt));if(oe===I)return;i.preventDefault();const tt=P[oe];(wt=O.current[tt])==null||wt.focus();return}if(i.key==="ArrowLeft"||i.key==="ArrowRight"){const I=P.indexOf(c);if(I<0)return;const oe=Rt(i.currentTarget)==="rtl"?"ArrowLeft":"ArrowRight",tt=i.key===oe?1:-1,gt=Math.max(0,Math.min(P.length-1,I+tt));if(gt===I)return;i.preventDefault();const ao=P[gt];(bt=O.current[ao])==null||bt.focus();return}if(!kt(i.key)||mo(i)||(i.preventDefault(),i.repeat))return;W(c),Je.current=c,J.current=Date.now();const k=i.currentTarget.ownerDocument.defaultView??window;$.current!==null&&(Q.current??k).clearTimeout($.current),Q.current=k,$.current=k.setTimeout(()=>{X()},ht),yt()},onKeyUp:i=>{i.altKey||i.ctrlKey||i.metaKey||kt(i.key)&&W(k=>k===c?null:k)},style:{border:no?"1px solid color-mix(in srgb, var(--aurora-accent-default) 48%, transparent)":"1px solid transparent",borderRadius:"var(--aurora-radius-sm)",background:dt?"color-mix(in srgb, var(--aurora-surface-elevated) 68%, var(--aurora-surface-default))":ut?"color-mix(in srgb, var(--aurora-surface-elevated) 86%, var(--aurora-surface-default))":"transparent",font:"inherit",cursor:j?"not-allowed":"pointer",display:"inline-flex",alignItems:"center",gap:6,padding:"2px 6px",color:j?"color-mix(in srgb, var(--aurora-text-secondary) 88%, var(--aurora-text-primary))":"inherit",boxShadow:mt?"0 0 0 3px color-mix(in srgb, var(--aurora-focus-ring) 42%, transparent)":"none",transform:dt?"translateY(1px)":"translateY(0)",transition:"background-color var(--aurora-motion-duration-fast) var(--aurora-motion-easing-standard), border-color var(--aurora-motion-duration-fast) var(--aurora-motion-easing-standard), box-shadow var(--aurora-motion-duration-fast) var(--aurora-motion-easing-standard), transform var(--aurora-motion-duration-fast) var(--aurora-motion-easing-standard)"},children:[s.header,o.jsx("span",{"aria-hidden":"true",children:T==="asc"?"▲":T==="desc"?"▼":"↕"})]}):s.header},c)})})}),o.jsx("tbody",{children:D?o.jsx("tr",{children:o.jsx("td",{colSpan:ct,style:{padding:"16px 12px",color:"var(--aurora-text-secondary)",textAlign:"center"},children:o.jsx("span",{role:"status","aria-live":"polite",children:za})})}):V.length===0?o.jsx("tr",{children:o.jsx("td",{colSpan:ct,style:{padding:"16px 12px",color:"var(--aurora-text-secondary)",textAlign:"center"},children:o.jsx("span",{role:"status","aria-live":"polite",children:Va})})}):V.map((s,l)=>{const u=s.row,c=String(s.sourceIndex),b=Ja[s.sourceIndex]??c;return o.jsx("tr",{style:{background:l%2===0?"transparent":"var(--aurora-surface-elevated)"},children:e.map((x,B)=>{const A=x.render?x.render(u,l,s.sourceIndex):String(u[String(x.key)]??""),T=C[B]??`${String(x.key)}__index-${B}`,K={padding:"10px 12px",borderBottom:"1px solid var(--aurora-border-default)",color:"var(--aurora-text-primary)",fontSize:"var(--aurora-font-size-sm)",textAlign:x.align??"left"};return x.rowHeader?o.jsx("th",{scope:"row",style:{...K,fontWeight:"var(--aurora-font-weight-medium)"},children:A},T):o.jsx("td",{style:K,children:A},T)})},b)})})]})]})}function Bt({columnHeader:e,nextDirection:t}){return`${e} sort ${t==="asc"?"ascending":"descending"}`}function uo({columnHeader:e,direction:t}){return`Sorted by ${e} ${t==="asc"?"ascending":"descending"}.`}function kt(e){return e==="Enter"||e===" "||e==="Space"||e==="Spacebar"}function mo(e){const t=e.nativeEvent;return t.isComposing?!0:typeof t.keyCode=="number"&&t.keyCode===229}function nt(e){return typeof e!="number"||e<=0}function yo(e,t){if(!e)return t;try{return e.matches(":focus-visible")||t}catch{return t}}function Rt(e){var v;const n=(e.ownerDocument.defaultView??window).getComputedStyle(e).direction;return n==="rtl"||n==="ltr"?n:((v=e.closest("[dir]"))==null?void 0:v.getAttribute("dir"))==="rtl"?"rtl":"ltr"}function po(e){if(typeof e=="number")return Number.isFinite(e)&&e>0?e:560;if(typeof e=="string"){const t=e.trim();if(t.length>0)return t}return 560}function Ve(e){return e==null||typeof e=="boolean"?!1:typeof e=="string"?e.trim().length>0:typeof e=="number"?!0:Array.isArray(e)?e.some(t=>Ve(t)):m.isValidElement(e)}function at(e){if(typeof e=="string")return e;if(typeof e=="number")return String(e);if(Array.isArray(e))return _e(e.map(n=>at(n)).filter(n=>n.length>0).join(" "));if(!m.isValidElement(e))return"";const t=e.props;if(t["aria-hidden"]===!0||t["aria-hidden"]==="true")return"";if(typeof t["aria-label"]=="string"){const n=_e(t["aria-label"]);if(n.length>0)return n}return at(t.children)}function _e(e){return e.replace(/\s+/g," ").trim()}function re(e,t){return typeof e=="string"&&e.trim().length>0?e.trim():t}function wo(e,t){if(typeof e.scrollBy=="function"){e.scrollBy({left:t,behavior:"auto"});return}e.scrollLeft+=t}function bo(e,t){if(typeof e.scrollTo=="function"){e.scrollTo({left:t,behavior:"auto"});return}e.scrollLeft=t}d.__docgenInfo={description:"",methods:[],displayName:"Table",props:{columns:{required:!0,tsType:{name:"Array",elements:[{name:"signature",type:"object",raw:`{
  key: keyof T | string;
