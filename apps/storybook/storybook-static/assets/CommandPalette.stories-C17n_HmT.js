import{j as n}from"./jsx-runtime-BjG_zV1W.js";import{r as s}from"./index-BWu4c2F4.js";import{D as It}from"./Dialog-CFqGnmxp.js";import{I as Pt}from"./Input-B0g8SV4q.js";import{within as y,expect as o,userEvent as i,fireEvent as R,waitFor as ve}from"./index-DgAF9SIF.js";import"./bodyScrollLock-DtRy56KP.js";import"./index-DlVbWVVj.js";import"./DismissableLayer-BWoUnDZk.js";import"./FieldStyles-CQy9UjBZ.js";import"./a11y-CU5SQrEA.js";function p({open:a,onOpenChange:e,commands:t,title:c="Command Palette",description:l="Quickly search and execute workspace actions.",searchAriaLabel:m="Search commands",resultsAriaLabel:x="Command results",closeOnSelect:b=!0,clearQueryOnEscape:E=!0,closeOnEscape:P=!0,closeOnOutsidePointer:se=!0,onEscapeKeyDown:ee,onPointerDownOutside:te,closeLabel:Ct,placeholder:Bt="Search commands...",emptyMessage:Tt="No commands found.",onQueryChange:h,onCloseReason:k,getResultsStatusText:ie=Ht}){const[C,ae]=s.useState(""),[S,f]=s.useState(0),ce=s.useRef(null),I=s.useRef(null),A=s.useRef(null),le=s.useRef(a),D=s.useId(),de=s.useId(),ue=s.useRef(null),H=s.useCallback(r=>{A.current=r,k==null||k(r)},[k]),me=s.useCallback(r=>{H(r),e(!1),A.current=null},[H,e]),Et=s.useCallback(r=>{!r&&a&&A.current===null&&(k==null||k("close-button")),A.current=null,e(r)},[k,e,a]);s.useEffect(()=>{var r;a?(r=ce.current)==null||r.focus():le.current&&(ae(d=>(d.length>0&&(h==null||h("")),"")),f(0)),le.current=a},[h,a]);const ne=s.useMemo(()=>be(C.trim()),[C]),u=s.useMemo(()=>ne?t.filter(r=>{const w=[St(r),...r.keywords??[]].join(" ");return be(w).includes(ne)}):t,[t,ne]),j=s.useMemo(()=>u.reduce((r,d)=>d.disabled?r:r+1,0),[u]),pe=u.length>0,Dt=s.useMemo(()=>{const r=["ArrowDown","ArrowUp","Home","End","PageDown","PageUp"];return j>0&&r.push("Enter"),(P||E&&C.length>0)&&r.push("Escape"),r.join(" ")},[E,P,j,C.length]),Rt=s.useMemo(()=>ie({query:C,visibleCount:u.length,enabledCount:j,totalCount:t.length}),[t.length,j,u.length,ie,C]),B=s.useMemo(()=>u.reduce((r,d,w)=>(d.disabled||r.push(w),r),[]),[u]),T=B[0]??-1,ye=B[B.length-1]??-1,g=s.useMemo(()=>{if(S<0)return-1;const r=u[S];return!r||r.disabled?-1:S},[S,u]);s.useEffect(()=>{var d;if(u.length===0){I.current=null,f(-1);return}const r=I.current;if(r){const w=u.findIndex(v=>!v.disabled&&v.key===r);if(w>=0){f(w);return}}f(T),I.current=T>=0?((d=u[T])==null?void 0:d.key)??null:null},[u,T]),s.useEffect(()=>{var r;if(g<0){I.current=null;return}I.current=((r=u[g])==null?void 0:r.key)??null},[u,g]),s.useEffect(()=>{if(!a||g<0)return;const r=ue.current,d=document.getElementById(`${D}-option-${g}`);!r||!d||!r.contains(d)||typeof d.scrollIntoView=="function"&&d.scrollIntoView({block:"nearest"})},[D,a,g]);const oe=s.useCallback(r=>{var w;const d=u[r];!d||d.disabled||((w=d.onSelect)==null||w.call(d),b&&me("item-select"))},[b,me,u]),we=r=>{var w;if(u.length===0)return;let d=g<0?r===1?-1:0:g;for(let v=0;v<u.length;v+=1)if(d=(d+r+u.length)%u.length,!((w=u[d])!=null&&w.disabled)){f(d);return}},ge=r=>{if(B.length===0)return;const d=B.indexOf(g);if(d<0){f(r===1?T:ye);return}const w=Math.min(B.length-1,Math.max(0,d+r*5));f(B[w]??T)};return n.jsx(It,{open:a,onOpenChange:Et,title:c,description:l,closeLabel:Ct,size:"md",closeOnEscape:P,closeOnOutsidePointer:se,onEscapeKeyDown:r=>{ee==null||ee(r),!(r.defaultPrevented||!P)&&H("escape-key")},onPointerDownOutside:r=>{te==null||te(r),!(r.defaultPrevented||!se)&&H("outside-pointer")},children:n.jsxs("div",{style:{display:"grid",gap:10},children:[n.jsx(Pt,{ref:ce,role:"combobox","aria-expanded":a,"aria-haspopup":"listbox","aria-autocomplete":"list","aria-controls":pe?D:void 0,"aria-activedescendant":g>=0?`${D}-option-${g}`:void 0,"aria-describedby":de,"aria-keyshortcuts":Dt,placeholder:Bt,value:C,onChange:r=>{ae(r.target.value),h==null||h(r.target.value)},onKeyDown:r=>{if(!At(r)){if(r.key==="Escape"&&E&&C.length>0){r.preventDefault(),ae(""),h==null||h("");return}if(r.key==="ArrowDown"){r.preventDefault(),we(1);return}if(r.key==="ArrowUp"){r.preventDefault(),we(-1);return}if(r.key==="Home"){r.preventDefault(),f(T);return}if(r.key==="End"){r.preventDefault(),f(ye);return}if(r.key==="PageDown"){r.preventDefault(),ge(1);return}if(r.key==="PageUp"){r.preventDefault(),ge(-1);return}if(r.key==="Enter"&&g>=0){if(r.preventDefault(),r.repeat)return;oe(g)}}},"aria-label":m}),n.jsx("p",{id:de,role:"status","aria-live":"polite",style:{position:"absolute",width:1,height:1,padding:0,margin:-1,overflow:"hidden",clip:"rect(0, 0, 0, 0)",whiteSpace:"nowrap",border:0},children:Rt}),pe?n.jsx("div",{id:D,ref:ue,role:"listbox","aria-label":x,style:{maxHeight:280,overflow:"auto",display:"grid",gap:4},children:u.map((r,d)=>{const w=d===g;return n.jsx("div",{id:`${D}-option-${d}`,role:"option","aria-selected":w,"aria-disabled":r.disabled||void 0,"aria-posinset":d+1,"aria-setsize":u.length,tabIndex:-1,onMouseDown:v=>{v.preventDefault()},onMouseEnter:()=>{r.disabled||f(d)},onClick:()=>{r.disabled||oe(d)},onKeyDown:v=>{r.disabled||(v.key==="Enter"||v.key===" "||v.key==="Space"||v.key==="Spacebar")&&(v.preventDefault(),oe(d))},style:{border:"1px solid var(--aurora-border-default)",background:w?"color-mix(in srgb, var(--aurora-accent-default) 14%, var(--aurora-surface-default))":"var(--aurora-surface-default)",color:r.disabled?"color-mix(in srgb, var(--aurora-text-secondary) 60%, transparent)":"var(--aurora-text-primary)",borderRadius:8,height:36,padding:"0 10px",textAlign:"left",cursor:r.disabled?"not-allowed":"pointer"},children:r.label},r.key)})}):n.jsx("p",{style:{margin:0,color:"var(--aurora-text-secondary)"},children:Tt})]})})}function St(a){return typeof a.textValue=="string"?a.textValue.trim():typeof a.label=="string"?a.label.trim():""}function be(a){return a.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase()}function At(a){const e=a.nativeEvent;return e.isComposing?!0:typeof e.keyCode=="number"&&e.keyCode===229}function Ht({query:a,visibleCount:e,enabledCount:t,totalCount:c}){const l=a.trim();return l.length===0?t!==c?`${t} of ${c} command${c===1?"":"s"} available.`:`${c} command${c===1?"":"s"} available.`:e===0?`No commands match "${l}".`:t===0?`No enabled commands match "${l}".`:t!==e?`${t} of ${e} matching command${t===1?"":"s"} available for "${l}".`:`${e} command${e===1?"":"s"} found for "${l}".`}p.__docgenInfo={description:"",methods:[],displayName:"CommandPalette",props:{open:{required:!0,tsType:{name:"boolean"},description:""},onOpenChange:{required:!0,tsType:{name:"signature",type:"function",raw:"(open: boolean) => void",signature:{arguments:[{type:{name:"boolean"},name:"open"}],return:{name:"void"}}},description:""},commands:{required:!0,tsType:{name:"Array",elements:[{name:"signature",type:"object",raw:`{
  key: string;
...
}`,...(Pe=(Ie=L.parameters)==null?void 0:Ie.docs)==null?void 0:Pe.source}}};var Se,Ae,He;$.parameters={...$.parameters,docs:{...(Se=$.parameters)==null?void 0:Se.docs,source:{originalSource:`{
  render: () => <QueryTelemetryPalette />,
  play: async ({
    canvasElement
  }) => {
