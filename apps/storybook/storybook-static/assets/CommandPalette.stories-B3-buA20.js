import{j as o}from"./jsx-runtime-BjG_zV1W.js";import{r as c}from"./index-BWu4c2F4.js";import{B as H}from"./Button-B21ouHNm.js";import{P as Za}from"./Popover-CI5AwEb2.js";import{D as eo}from"./Dialog-Cbs75tsL.js";import{I as to}from"./Input-DCFTlK0_.js";import{within as d,userEvent as i,expect as n,fireEvent as g,waitFor as A}from"./index-DgAF9SIF.js";import{b as v,s as y,a as b}from"./storyShowcase-DlKirntE.js";import"./focusTabbable-gnwmp6b7.js";import"./bodyScrollLock-DU3WNas9.js";import"./index-DlVbWVVj.js";import"./FieldStyles-CQy9UjBZ.js";import"./a11y-CU5SQrEA.js";function u({open:a,onOpenChange:e,commands:t,title:s="Command Palette",description:l="Quickly search and execute workspace actions.",ariaLabel:p,ariaLabelledBy:w,searchAriaLabel:B="Search commands",resultsAriaLabel:U="Command results",closeOnSelect:Je=!0,clearQueryOnEscape:Ke=!0,closeOnEscape:V=!0,closeOnOutsidePointer:Xe=!0,onEscapeKeyDown:Ne,onSearchKeyDown:Me,onCloseButtonKeyDown:qa,onPointerDownOutside:Fe,closeLabel:Ka,placeholder:Na="Search commands...",loading:E=!1,loadingContent:G="Loading commands...",emptyMessage:Ze="No commands found.",disabledResultsMessage:et="Matching commands are currently unavailable.",onQueryChange:C,onCloseReason:D,getResultsStatusText:tt=ao}){const[S,$e]=c.useState(""),[z,T]=c.useState(0),Ma=c.useRef(null),M=c.useRef(null),Q=c.useRef(null),nt=c.useRef(a);c.useRef(null),c.useRef(null),c.useRef(null);const q=c.useId(),Ue=c.useId(),at=c.useId(),ot=c.useRef(null),Fa=L(B,"Search commands"),$a=L(U,"Command results"),Ua=L(Ka,"Close command palette"),st=L(w),Va=st===void 0?L(p):void 0,rt=c.useMemo(()=>oo(G),[G]),Ga=We(G)?G:"Loading commands...",za=We(Ze)?Ze:"No commands found.",Qa=We(et)?et:"Matching commands are currently unavailable.",W=c.useCallback(r=>{Q.current=r,D==null||D(r)},[D]),ct=c.useCallback(r=>{W(r),e(!1),Q.current=null},[W,e]),Wa=c.useCallback(r=>{!r&&a&&Q.current===null&&(D==null||D("close-button")),Q.current=null,e(r)},[D,e,a]);c.useEffect(()=>{!a&&nt.current&&($e(r=>(r.length>0&&(C==null||C("")),"")),T(0)),nt.current=a},[C,a]),c.useEffect(()=>{},[t]),c.useEffect(()=>{},[t]),c.useEffect(()=>{},[t]);const Ve=c.useMemo(()=>yt(S.trim()),[S]),Ge=c.useMemo(()=>{const r=new Map;t.forEach(h=>{r.set(h.key,(r.get(h.key)??0)+1)});const m=new Map;return t.map(h=>{const R=r.get(h.key)??0,j=m.get(h.key)??0;return m.set(h.key,j+1),{item:h,renderKey:R>1?`${h.key}__duplicate-${j}`:h.key}})},[t]),_=c.useMemo(()=>Ve?Ge.filter(({item:r})=>{const h=[pt(r),...r.keywords??[]].join(" ");return yt(h).includes(Ve)}):Ge,[Ge,Ve]),x=c.useMemo(()=>_.map(r=>r.item),[_]),K=c.useMemo(()=>_.map(r=>r.renderKey),[_]),F=c.useMemo(()=>x.reduce((r,m)=>m.disabled?r:r+1,0),[x]),it=!E&&x.length>0&&F===0,I=!E&&F>1,lt=!E&&F>0,ze=!E&&x.length>0,_a=c.useMemo(()=>{const r=[];return I&&r.push("ArrowDown","ArrowUp","Home","End","PageDown","PageUp"),lt&&r.push("Enter"),(V||Ke&&S.length>0)&&r.push("Escape"),r.length>0?r.join(" "):void 0},[lt,I,Ke,V,S.length]),Ya=c.useMemo(()=>E?rt:tt({query:S,visibleCount:x.length,enabledCount:F,totalCount:t.length}),[t.length,F,x.length,tt,E,S,rt]),Ja=it?`${Ue} ${at}`:Ue,P=c.useMemo(()=>x.reduce((r,m,h)=>(m.disabled||r.push(h),r),[]),[x]),O=P[0]??-1,dt=P[P.length-1]??-1,f=c.useMemo(()=>{if(z<0)return-1;const r=x[z];return!r||r.disabled?-1:z},[z,x]);c.useEffect(()=>{if(x.length===0){M.current=null,T(-1);return}const r=M.current;if(r){const m=K.findIndex((h,R)=>{var j;return!((j=x[R])!=null&&j.disabled)&&h===r});if(m>=0){T(m);return}}T(O),M.current=O>=0?K[O]??null:null},[x,K,O]),c.useEffect(()=>{if(f<0){M.current=null;return}M.current=K[f]??null},[K,f]),c.useEffect(()=>{if(!a||f<0)return;const r=ot.current;if(!r)return;const m=r.ownerDocument.getElementById(`${q}-option-${f}`);!m||!r.contains(m)||typeof m.scrollIntoView=="function"&&m.scrollIntoView({block:"nearest"})},[q,a,f]);const Qe=c.useCallback(r=>{const m=x[r];!m||m.disabled||m.onSelect&&(m.onSelect(),Je&&ct("item-select"))},[Je,ct,x]),ut=r=>{var h;if(x.length===0)return;let m=f<0?r===1?-1:0:f;for(let R=0;R<x.length;R+=1)if(m=(m+r+x.length)%x.length,!((h=x[m])!=null&&h.disabled)){T(m);return}},mt=r=>{if(P.length===0)return;const m=P.indexOf(f);if(m<0){T(r===1?O:dt);return}const h=Math.min(P.length-1,Math.max(0,m+r*5));T(P[h]??O)};return o.jsx(eo,{open:a,onOpenChange:Wa,title:s,description:l,ariaLabel:Va,ariaLabelledBy:st,closeLabel:Ua,size:"md",closeOnEscape:V,closeOnOutsidePointer:Xe,onCloseButtonKeyDown:qa,onEscapeKeyDown:r=>{Ne==null||Ne(r),!(r.defaultPrevented||!V)&&W("escape-key")},onPointerDownOutside:r=>{Fe==null||Fe(r),!(r.defaultPrevented||!Xe)&&W("outside-pointer")},children:o.jsxs("div",{style:{display:"grid",gap:10},children:[o.jsx(to,{ref:Ma,"data-autofocus":"",role:"combobox","aria-busy":E||void 0,"aria-expanded":ze,"aria-haspopup":"listbox","aria-autocomplete":"list","aria-controls":ze?q:void 0,"aria-activedescendant":!E&&f>=0?`${q}-option-${f}`:void 0,"aria-describedby":Ja,"aria-keyshortcuts":_a,placeholder:Na,value:S,onChange:r=>{$e(r.target.value),C==null||C(r.target.value)},onKeyDown:r=>{if(Me==null||Me(r),!no(r)&&!r.defaultPrevented&&!(r.altKey||r.ctrlKey||r.metaKey)){if(r.key==="Escape"){if(r.repeat)return;Ke&&S.length>0&&(r.preventDefault(),$e(""),C==null||C(""));return}if(r.key==="ArrowDown"){if(!I)return;r.preventDefault(),ut(1);return}if(r.key==="ArrowUp"){if(!I)return;r.preventDefault(),ut(-1);return}if(r.key==="Home"){if(!I||!wt(r))return;r.preventDefault(),T(O);return}if(r.key==="End"){if(!I||!wt(r))return;r.preventDefault(),T(dt);return}if(r.key==="PageDown"){if(!I)return;r.preventDefault(),mt(1);return}if(r.key==="PageUp"){if(!I)return;r.preventDefault(),mt(-1);return}if(r.key==="Enter"&&!E&&f>=0){if(r.preventDefault(),r.repeat)return;Qe(f)}}},"aria-label":Fa}),o.jsx("p",{id:Ue,role:"status","aria-live":"polite",style:{position:"absolute",width:1,height:1,padding:0,margin:-1,overflow:"hidden",clip:"rect(0, 0, 0, 0)",whiteSpace:"nowrap",border:0},children:Ya}),E?o.jsx("p",{"data-testid":"command-palette-loading-content",style:{margin:0,color:"var(--aurora-text-secondary)"},children:Ga}):ze?o.jsxs(o.Fragment,{children:[o.jsx("div",{id:q,ref:ot,role:"listbox","aria-label":$a,style:{maxHeight:280,overflow:"auto",display:"grid",gap:4},children:x.map((r,m)=>{const h=m===f,R=L(r.ariaLabelledBy),j=$(r.label).trim().length>0,Xa=R?void 0:L(r.ariaLabel,j?void 0:pt(r));return o.jsx("div",{id:`${q}-option-${m}`,role:"option","aria-selected":h,"aria-disabled":r.disabled||void 0,"aria-labelledby":R,"aria-label":Xa,"aria-keyshortcuts":r.disabled?void 0:"Enter Space","aria-posinset":m+1,"aria-setsize":x.length,tabIndex:-1,onPointerDown:k=>{k.pointerType==="mouse"||!so(k.button)||k.ctrlKey||k.preventDefault()},onMouseDown:k=>{k.button!==0||k.ctrlKey||k.preventDefault()},onMouseEnter:()=>{r.disabled||T(m)},onClick:()=>{r.disabled||Qe(m)},onKeyDown:k=>{if(!r.disabled&&!k.defaultPrevented&&!(k.altKey||k.ctrlKey||k.metaKey)&&(k.key==="Enter"||k.key===" "||k.key==="Space"||k.key==="Spacebar")){if(k.preventDefault(),k.repeat)return;Qe(m)}},style:{border:"1px solid var(--aurora-border-default)",background:h?"color-mix(in srgb, var(--aurora-accent-default) 14%, var(--aurora-surface-default))":"var(--aurora-surface-default)",color:r.disabled?"color-mix(in srgb, var(--aurora-text-secondary) 60%, transparent)":"var(--aurora-text-primary)",borderRadius:8,height:36,padding:"0 10px",textAlign:"left",cursor:r.disabled?"not-allowed":"pointer"},children:r.label},K[m]??`${r.key}__source-fallback-${m}`)})}),it?o.jsx("p",{id:at,"data-testid":"command-palette-disabled-results-message",style:{margin:0,color:"var(--aurora-text-secondary)",fontSize:13},children:Qa}):null]}):o.jsx("p",{style:{margin:0,color:"var(--aurora-text-secondary)"},children:za})]})})}function pt(a){if(typeof a.textValue=="string"){const e=N(a.textValue);if(e.length>0)return e}if(typeof a.ariaLabel=="string"){const e=N(a.ariaLabel);if(e.length>0)return e}return N($(a.label))}function yt(a){return a.normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g," ").trim().toLowerCase()}function no(a){const e=a.nativeEvent;return e.isComposing?!0:typeof e.keyCode=="number"&&e.keyCode===229}function wt(a){if(a.key!=="Home"&&a.key!=="End")return!0;const e=a.currentTarget,t=e.selectionStart,s=e.selectionEnd;return t===null||s===null?!0:t!==s?!1:a.key==="Home"?t===0:s===e.value.length}function ao({query:a,visibleCount:e,enabledCount:t,totalCount:s}){const l=a.trim();return l.length===0?t!==s?`${t} of ${s} command${s===1?"":"s"} available.`:`${s} command${s===1?"":"s"} available.`:e===0?`No commands match "${l}".`:t===0?`No enabled commands match "${l}".`:t!==e?`${t} of ${e} matching command${t===1?"":"s"} available for "${l}".`:`${e} command${e===1?"":"s"} found for "${l}".`}function oo(a){const e=N($(a));return e.length>0?e:"Loading commands..."}function We(a){return a==null||typeof a=="boolean"?!1:typeof a=="string"?a.trim().length>0:!0}function $(a){if(typeof a=="string")return a;if(typeof a=="number")return String(a);if(Array.isArray(a))return N(a.map(t=>$(t)).filter(t=>t.length>0).join(" "));if(!c.isValidElement(a))return"";const e=a.props;return e["aria-hidden"]===!0||e["aria-hidden"]==="true"?"":typeof e["aria-label"]=="string"&&e["aria-label"].trim().length>0?N(e["aria-label"]):$(e.children)}function L(a,e){return typeof a=="string"&&a.trim().length>0?a.trim():e}function so(a){return typeof a!="number"||a<=0}function N(a){return a.replace(/\s+/g," ").trim()}u.__docgenInfo={description:"",methods:[],displayName:"CommandPalette",props:{open:{required:!0,tsType:{name:"boolean"},description:""},onOpenChange:{required:!0,tsType:{name:"signature",type:"function",raw:"(open: boolean) => void",signature:{arguments:[{type:{name:"boolean"},name:"open"}],return:{name:"void"}}},description:""},commands:{required:!0,tsType:{name:"Array",elements:[{name:"signature",type:"object",raw:`{
  key: string;
...
}`,...(Gt=(Vt=se.parameters)==null?void 0:Vt.docs)==null?void 0:Gt.source}}};var zt,Qt,Wt;re.parameters={...re.parameters,docs:{...(zt=re.parameters)==null?void 0:zt.docs,source:{originalSource:`{
  render: () => <QueryTelemetryPalette />,
  play: async ({
    canvasElement
  }) => {
...
}`,...(zn=(Gn=fe.parameters)==null?void 0:Gn.docs)==null?void 0:zn.source}}};var Qn,Wn,_n;Ee.parameters={...Ee.parameters,docs:{...(Qn=Ee.parameters)==null?void 0:Qn.docs,source:{originalSource:`{
